...
<a class="brand" href="/"><picture><source type="image/avif" srcset="/assets/images/sulaksh-emblem-38.avif 38w, /assets/images/sulaksh-emblem-76.avif 76w" sizes="34px"><source type="image/webp" srcset="/assets/images/sulaksh-emblem-38.webp 38w, /assets/images/sulaksh-emblem-76.webp 76w" sizes="34px"><img class="e" src="/assets/images/sulaksh-emblem-44.webp" alt="Sulaksh - Delhi University previous year question papers and study material" width="44" height="37" loading="eager" decoding="async"></picture><picture><source type="image/avif" srcset="/assets/images/sulaksh-wordmark-100.avif 100w, /assets/images/sulaksh-wordmark-200.avif 200w" sizes="100px"><source type="image/webp" srcset="/assets/images/sulaksh-wordmark-100.webp 100w, /assets/images/sulaksh-wordmark-200.webp 200w" sizes="100px"><img class="w" src="/assets/images/sulaksh-wordmark-140.webp" alt="SULAKSH - DU PYQs syllabus notes free download" width="140" height="25" loading="eager" decoding="async"></picture></a>
...
<!-- Default Statcounter code for https://sulaksh.online -->
<script type="text/javascript">
var sc_project=13355367; 
var sc_invisible=1; 
var sc_security="53812362"; 
</script>
<script type="text/javascript" src="https://www.statcounter.com/counter/counter.js" async></script>
<noscript><div class="statcounter"><a title="Web Analytics" href="https://statcounter.com/" target="_blank"><img class="statcounter" src="https://c.statcounter.com/13355367/0/53812362/1/" alt="Web Analytics" referrerPolicy="no-referrer-when-downgrade"></a></div></noscript>
...
for (const [key, semMap] of bySubjTrack) {
  ...
  let total = [...semMap.values()].reduce((a, v) => a + v.length, 0);
  const semCount = semMap.size;
  total = honestCount(total);
  ...
  `;
  const hubDisplay = displayLabel(total, !!aboutBlock);
  emit(ovFile(subject, track),
    `${subject} ${track} PYQs, Syllabus & Notes – DU ${honestCount(total)} Docs | Sulaksh`,
  ...
  for (const [semName, arr] of semMap) {
    ...
    const displayCountSem = honestCount(arr.length);
    const semHasGuide = true;
    ...
    for (const [t, arr2] of types) {
      ...
      const displayCountType = honestCount(arr2.length);
  ...
  for (const [t, arr] of typesAll) {
    const displayCountAll = honestCount(arr.length);
...
for (const [k, v] of ncCombined) {
  ...
  const honestTotalNC = honestCount(v.items.length);
  let aboutNC = uniqueAboutBlock(v.subject, v.label, honestTotalNC, 1);
...
emit('where-to-find-du-syllabus.html',
  ...
  `<h2>Official Source First</h2>
   ...
   <p>Official sources: <a href="https://www.du.ac.in" target="_blank" rel="noopener" style="color:var(--blue); text-decoration: underline; text-underline-offset: 2px;">University of Delhi</a> · <a href="http://exam.du.ac.in" target="_blank" rel="noopener" style="color:var(--blue); text-decoration: underline; text-underline-offset: 2px;">DU Exam Portal</a> · your college handout. Verify the final unit list and paper code from your college — the broad overview here is a bridge until the exact PDF is uploaded and will be replaced by the verified semester-wise PDF.</p>
...
emit('best-website-for-du-pyqs-study-material.html',
  ...
  `<h2>Your Options, Ranked</h2>
   ...
   <p>Official sources: <a href="https://www.du.ac.in" target="_blank" rel="noopener" style="color:var(--blue); text-decoration: underline; text-underline-offset: 2px;">University of Delhi</a> · <a href="http://exam.du.ac.in" target="_blank" rel="noopener" style="color:var(--blue); text-decoration: underline; text-underline-offset: 2px;">DU Exam Portal</a> · your college handout. Verify the final unit list and paper code from your college.</p>
...
emit('index.html',
  ...
  `<div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:14px;margin:0 0 18px"><label for="archSearch" style="font-size:14px;font-weight:800;display:block;margin-bottom:8px">🔍 Search the 23,000+ PYQ Archive</label><input id="archSearch" type="search" placeholder="Type paper name — e.g. data privacy, corporate accounting…" autocomplete="off" style="width:100%;padding:11px 14px;border:1px solid var(--border);border-radius:8px;font-size:14.5px;background:var(--bg);color:var(--text);outline:none"><p style="font-size:12.5px;color:var(--muted);margin:8px 0 0">💡 Tip: use this search for best results — type the exact paper name to see every matching PYQ across years. Browsing below is by course; searching finds the paper directly.</p><p id="archCount" style="font-size:12.5px;color:var(--muted);margin:8px 0 0"></p><div id="archResults"></div><button id="archMore" style="display:none;width:100%;margin-top:4px;background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:10px;font-weight:700;font-size:13.5px;cursor:pointer"></button></div>
   ...
   (function(){
      ...
      // Warm up the API while the visitor reads/types (free-tier servers sleep;
      // first cold request is the #1 cause of "snag" on someone else's phone).
      fetch(API+'/api/health').catch(function(){});
      // Retry with backoff: one failed fetch (sleeping server, train tunnel,
      // Brave shields hiccup) must not end in a dead "snag".
      function fetchRetry(url, tries){
        tries=tries||3;
        function attempt(n){
          return fetch(url).catch(function(e){
            if(n<=1) throw e;
            return new Promise(function(res){ setTimeout(function(){ res(attempt(n-1)); }, 900); });
          });
        }
        return attempt(tries);
      }
      // Failure card: tappable retry + the Brave/ad-block hint (second cause).
      function snag(retryFn){
        meta.innerHTML='Search hit a snag — <u>tap to retry</u>. Tip: Brave shields or ad-blockers sometimes block the archive; allow this site and retry.';
        meta.style.cursor='pointer';
        meta.onclick=function(){ meta.onclick=null; meta.style.cursor=''; retryFn(); };
      }
      function clearSnag(){ meta.onclick=null; meta.style.cursor=''; }
      // Honor the admin "Downloads enabled" toggle: hide every Save button when
      // downloads are off (View always stays). Defaults to visible until the
      // config check resolves, matching the rest of the site.
      var DL_ON=true;
      function hideDl(){ if(DL_ON) return; var els=document.querySelectorAll('#archResults [data-dl], #featList [data-dl]'); for(var i=0;i<els.length;i++){ els[i].style.display='none'; } }
      fetch(API+'/api/config').then(function(r){return r.json();}).then(function(c){ if(c&&c.downloads_enabled===false){ DL_ON=false; hideDl(); } }).catch(function(){});
     ...
     function row(m){
       ...
       return '<div style="display:flex;gap:10px;align-items:center;justify-content:space-between;background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:10px 14px;margin:8px 0;font-size:13.5px">'
         ...
         + '<div style="font-size:12px;color:var(--muted)">'+esc(bits)+'</div></div>'
          + '<div style="display:flex;gap:6px;flex-shrink:0"><a href="/view.html?v=4&id='+encodeURIComponent(m.id)+'" style="background:var(--navy);color:#fff;border-radius:8px;padding:7px 13px;font-weight:700;font-size:12.5px">View</a>'
          + (DL_ON?'<button data-dl="'+esc(m.id)+'" style="background:var(--bg);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:7px 13px;font-weight:700;font-size:12.5px;cursor:pointer">Save</button>':'') + '</div></div>';
      }
      ...
      function doSearch(q){
        ...
        var eff=ALIAS[raw]||q;
        fetchRetry(API+'/api/materials?exam='+encodeURIComponent('PYQ Archive')+'&q='+encodeURIComponent(eff)+'&limit=20')
          .then(function(r){return r.json();}).then(function(d){
            ...
            box.innerHTML=arr.map(row).join(''); hideDl();
          }).catch(function(){ snag(function(){ doSearch(q); }); });
      }
      ...
      more.addEventListener('click', function(){
        more.style.display='none'; clearSnag(); meta.textContent='Loading all matches…';
        fetchRetry(API+'/api/materials?exam='+encodeURIComponent('PYQ Archive')+'&q='+encodeURIComponent(lastEff)+'&limit=100')
          .then(function(r){return r.json();}).then(function(d){
            ...
            box.innerHTML=arr.map(row).join(''); hideDl();
          }).catch(function(){ snag(function(){ more.click(); }); });
      });
   })();
   </script>
...
for (const silo of siloDefs) {
  ...
  <p>Official sources: <a href="https://www.du.ac.in" target="_blank" rel="noopener" style="color:var(--blue); text-decoration: underline; text-underline-offset: 2px;">University of Delhi</a> · <a href="http://exam.du.ac.in" target="_blank" rel="noopener" style="color:var(--blue); text-decoration: underline; text-underline-offset: 2px;">DU Exam Portal</a> · your college handout. The verified PDF will auto-appear when admin uploads.</p>`;
