...
router.post('/materials', upload.single('file'), async (req, res) => {
  const { title, exam, category, subject, track, year, description, is_imp, is_syllabus, is_pyq } = req.body || {};
  ...
  try {
    db.prepare(`
      INSERT INTO materials
        (id, title, exam, category, subject, track, year, description, file_name, file_size, content_type, r2_object_key, status, is_imp, is_syllabus, is_pyq, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `    ).run(
      id, title, exam, category, subject || null, track || null, year ? parseInt(year, 10) : null,
...
router.patch('/materials/:id', (req, res) => {
  ...
  const fields = ['title', 'exam', 'category', 'subject', 'track', 'year', 'description', 'is_imp', 'is_syllabus', 'is_pyq'];
  const updates = {};
